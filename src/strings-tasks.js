...
function isString(value) {
  let val1 = value;
  if (typeof value === 'object' && value !== null) val1 = value.valueOf();
...
function countVowels(str) {
  ...
  let num = 0;
  for (let i = 0; i < str.length; i += 1) {
    if (vowels.includes(str.charAt(i).toString())) num += 1;
  }
  return num;
}
...
function isPalindrome(str) {
  const strRivert = str
    ...
    .join('')
    .toLowerCase();
  const st = str.replace(/[\s.,!,?]/g, '').toLowerCase();
  return st === strRivert;
}
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < strArr.length; i += 1) {
    if (word.length < strArr[i].length) word = strArr[i];
  }
  return word;
}
...
function encodeToRot13(str) {
  ...
  let stResult = '';
  for (let i = 0; i < str.length; i += 1) {
    if (!arr1.includes(str.charAt(i))) {
      stResult += str.charAt(i);
    }
    for (let j = 0; j < arr1.length; j += 1) {
      if (str.charAt(i) === arr1.charAt(j)) {
        stResult += arr2.charAt(j);
      }
    }
  }
  return stResult;
